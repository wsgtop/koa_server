...
const captycha = () => {
  ...
  const result = svgCaptycha.create({background:'#cc9966',size:6,ignoreChars:'oO01iI',noise:'4',color:true}) 
 ...
 */
const captychaExpr = () => {
  const result = svgCaptycha.createMathExpr({background:'#cc9966',size:6,ignoreChars:'oO01iI',noise:'4',color:true}) 
  const {text,data} = result
  logger.info('此次生成验证码',text)
  return data
}
...
const captychaDIY = () => {
  const result = svgCaptycha.create({background:'#cc9966',size:6,ignoreChars:'oO01iI',noise:'4',color:true,width:55,height:20,fontSize:16}) 
  const {text,data} = result
  logger.info('此次生成验证码',text)
  return data
}
